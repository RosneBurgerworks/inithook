...
public:
    ...
    DynamicBytePatch(std::function<uintptr_t(const char *)> SigScanFunc, const char *pattern, size_t offset, std::vector<unsigned char> patch) : patch_bytes{ patch }
    {
    ...
    DynamicBytePatch(uintptr_t addr, std::vector<unsigned char> patch) : addr{ reinterpret_cast<void *>(addr) }, patch_bytes{ patch }
    {
        size = patch.size();
        original.resize(size);
        memcpy(&original[0], reinterpret_cast<void *>(addr), size);
    }
    DynamicBytePatch(void *addr, std::vector<unsigned char> patch) : addr{ addr }, patch_bytes{ patch }
    {
        size = patch.size();
        original.resize(size);
        memcpy(&original[0], addr, size);
