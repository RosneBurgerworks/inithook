...
extern rgba_t background;                              // Background color for visuals
...
constexpr rgba_t purple = FromRGBA8(204, 0, 255, 255);
...
constexpr rgba_t Health(int health, int max)
{
    ...
    // If More than 50% Health Set Red to 100% (Yes the <= 0.5f is "More than 50%" in this
    // case)
    if (hf <= 0.5f)
        to_return.r = 1.0f;
...
rgba_t EntityF(CachedEntity *ent, float alpha = 1.0f);
} // namespace colors
