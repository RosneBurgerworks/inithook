...
static CatCommand cat("cat", "", [](const CCommand &args) {
    if (args.ArgC() < 3)
    {
        logging::InfoConsole("Usage: cat <set/get> <variable> [value]");
        return;
    }
    ...
    if (variable == nullptr)
    {
        logging::InfoConsole("Variable not found: %s", args.Arg(2));
        return;
    }
    ...
    if (!strcmp(args.Arg(1), "set"))
    {
        if (args.ArgC() < 4)
        {
            logging::InfoConsole("Usage: cat <set> <variable> <value>");
            return;
        }
        variable->fromString(args.Arg(3));
        logging::InfoConsole("%s = \"%s\"", args.Arg(2), variable->toString().c_str());
        if (autosave)
        {
...
static CatCommand toggle("toggle", "", [](const CCommand &args) {
    if (args.ArgC() < 4)
    {
        logging::InfoConsole("Usage: cat_toggle <variable> <value 1> <value 2>");
        return;
    }
    ...
    if (variable == nullptr)
    {
        logging::InfoConsole("Variable not found: %s\n", rvar.c_str());
        return;
    }
...
static CatCommand load("load", "", [](const CCommand &args) {
    ...
    if (args.ArgC() == 1)
    {
        loader.loadFrom(std::string(DATA_PATH "/configs") + "/default.conf");
    }
    else
    {
...
static CatCommand cat_find("find", "Find a command by name", [](const CCommand &args) {
    ...
    // Yes
    logging::InfoConsole("Found rvars:");
    // Nothing found :C
    if (found_rvars.empty())
        logging::InfoConsole("No rvars found.");
    // Found rvars
    else
        for (auto &s : found_rvars)
            logging::InfoConsole("%s", s.c_str());
});
