...
bool settings::SettingsWriter::saveTo(std::string path, bool autosave)
{
    ...
    {
        if (!autosave)
            logging::InfoConsole("CAT: cat_save: Can't create config file!");
        return false;
    }
    ...
    {
        if (!autosave)
            logging::InfoConsole("cat_save: Failed to save config!");
    }
    else if (!autosave)
        logging::InfoConsole("cat_save: Successfully saved config!");
    stream.close();
    return true;
}
...
bool settings::SettingsReader::loadFrom(std::string path)
{
    ...
    if (stream.fail())
    {
        logging::InfoConsole("cat_load: File doesn't exist / can't open file!");
        return false;
    }
    ...
    {
        logging::InfoConsole("cat_load: FATAL: Read failed!");
        return false;
    }

    logging::InfoConsole("cat_load: Successfully loaded config!");
    finishString(true);
...
bool settings::SettingsReader::loadFromString(std::string stream)
{
    ...
    if (stream == "")
    {
        logging::InfoConsole("cat_load: Empty String!");
        return false;
    }
