...
static CatCommand print("dc_print", "Print codes of all available data centers", []() {
    ...
    if (count <= 0)
    {
        logging::InfoConsole("List of regions is not available yet");
        return;
    }
    auto *list = new SteamNetworkingPOPID[count];
...
static void Init()
{
    enable.installChangeCallback([](bool on) {
        auto addr       = gSignatures.GetClientSignature("c7 04 ? ? ? ? ? 89 ? ? ? ? ? 65 a1 14 00 00 00 89 ? ? 31 c0 e8 ? ? ? ? 8b 00");
        auto createArg  = *(void **) (addr + 0x03);
        auto createFunc = (void **(*) (void *) ) e8call_direct(addr + 0x18);
        if (!g_ISteamNetworkingUtils)
        {
            g_ISteamNetworkingUtils = *createFunc(createArg);
            if (!g_ISteamNetworkingUtils)
            {
    ...
    regions.installChangeCallback([](std::string regions) { OnRegionsUpdate(regions); });
    restrict.installChangeCallback([](bool) { Refresh(); });
    enable_eu.installChangeCallback([](bool after) { manageRegions(eu_datacenters, after); });
    enable_north_america.installChangeCallback([](bool after) { manageRegions(north_america_datacenters, after); });
    enable_south_america.installChangeCallback([](bool after) { manageRegions(south_america_datacenters, after); });
    enable_asia.installChangeCallback([](bool after) { manageRegions(asia_datacenters, after); });
    enable_oceania.installChangeCallback([](bool after) { manageRegions(oceana_datacenters, after); });
    enable_africa.installChangeCallback([](bool after) { manageRegions(africa_datacenters, after); });
}
