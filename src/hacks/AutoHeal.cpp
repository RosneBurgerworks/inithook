...
/* static settings::Boolean pop_uber_voice{ "autoheal.popvoice", "true" }; FIXME: This is broken */
...
int BulletDangerValue(CachedEntity *patient)
{
    ...
    for (int i = 1; i <= g_IEngine->GetMaxClients(); i++)
    {
        ...
        else
        {
            ...
            {
                if (playerlist::AccessData(ent).state == playerlist::EState::RAGE)
                    return 2;
                else
                {
...
int HealingPriority(int idx)
{
    ...
    {
    case playerlist::EState::PARTY:
    case playerlist::EState::FRIEND:
