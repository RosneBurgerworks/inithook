...
static settings::Button latency_key{ "backtrack.latency.key", "<null>" };
static settings::Int latency_key_mode{ "backtrack.latency.key.mode", "0" };
#endif
...
bool LatencyKey()
{
    ...
    if (latency_key && latency_key_mode)
    {
        // Grab whether the key is depressed
        bool key_down = latency_key.isKeyDown();
        switch (*latency_key_mode)
        {
...
float getLatency()
{
    ...
    // Fix the latency
    float backtrack_latency = LatencyKey() ? *latency : 0.0f - real_latency;
...
bool defaultTickFilter(CachedEntity *ent, BacktrackData tick)
{
    ...
    if (g_pLocalPlayer->weapon_mode != weapon_hitscan)
        return true;
    // Return visibility
    return IsEntityVectorVisible(ent, tick.hitboxes.at(head).center);
}
