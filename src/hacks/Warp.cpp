...
static settings::Boolean rf_use_all_ticks{ "warp.rapidfire.force-all-ticks", "true" };
...
static settings::Int rf_disable_on{ "warp.rapidfire.disable-on", "3" };
...
static settings::Int maxusrcmdprocessticks("warp.maxusrcmdprocessticks", "25");
...
bool shouldRapidfire()
{
    ...
    if (!rf_use_all_ticks && (warp_amount < TIME_TO_TICKS(getFireDelay()) && (TIME_TO_TICKS(getFireDelay()) < *maxusrcmdprocessticks - 1 || (wait_full && warp_amount != GetMaxWarpTicks()))))
        return false;
...
void warpLogic()
{
    ...
    if (!shouldWarp(false))
    {
        ...
        Vector velocity{};
        re::C_BaseEntity::EstimateAbsVelocity(RAW_ENT(LOCAL_E), velocity);
...
void Draw()
{
    ...
    if (draw_bar)
    {
        ...
        draw::Rectangle(*bar_x - 5.0f, *bar_y - 5.0f, bar_bg_x_size + 10.0f, bar_bg_y_size + 10.0f, colors::background);
        // Draw bar
