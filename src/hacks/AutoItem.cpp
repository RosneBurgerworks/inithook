...
static settings::Int hat1_normal{ "auto-item.hats.1", "940" };
static settings::Int hat2_normal{ "auto-item.hats.2", "941" };
static settings::Int hat3_normal{ "auto-item.hats.3", "302" };
...
// Achievement hats we pick from at init for randomization
static std::vector<int> achievement_hats{ 940, 302, 668, 756, 941, 744, 1164, 1169 };
...
void getAndEquipWeapon(std::string str, int clazz, int slot)
{
    ...
    // Single item, rent or simply get via achievement, use fallback if needed and
    // specified.
    ...
    else
    {
        ...
        if (loc != std::string::npos)
        {
            ...
            for (auto &group_str : craft_groups[slot])
            {
                ...
                // See if we have the requirements to perform this craft, if not try to
                // get them.
                    ...
                    else
                    {
                        ...
                        if (index)
                        {
                            ...
                            if (g_IAchievementMgr->HasAchieved(index->name.c_str()))
                            {
                                ...
                                // User's fault if they put a ach item they unlocked and
                                // have used for crafting already/deleted.
                                rec_req_amount_have++;
                                continue;
                            }
...
void CreateMove()
{
    ...
    if (clazz != 0)
    {
        ...
        if (hats)
        {
            ...
            equipItem(clazz, slots[offset], *hats_random ? random_hats[0] : *hat1_normal);
            equipItem(clazz, slots[(offset + 1) % 3], *hats_random ? random_hats[1] : *hat2_normal);
            equipItem(clazz, slots[(offset + 2) % 3], *hats_random ? random_hats[2] : *hat3_normal);
            offset = (offset + 1) % 3;
        }
...
static InitRoutine init([]() {
    primary.installChangeCallback([](std::string after) { rvarCallback(after, 0); });
    secondary.installChangeCallback([](std::string after) { rvarCallback(after, 1); });
    melee.installChangeCallback([](std::string after) { rvarCallback(after, 2); });
    setupRandomHats();
