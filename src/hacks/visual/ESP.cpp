...
static settings::Float thicc{ "esp.thicczzz", "0.5" };
static settings::Float thicc2{ "esp.thicczzz.border", "4.0" };
...
static settings::Boolean entity_player_flsim{ "esp.players.debug-flsimtime", "false" };
static settings::Boolean identify_only{ "esp.identify-only", "false" };
        ...
        if (ent->m_Type() == ENTITY_PLAYER)
        {
            ...
            if (draw::WorldToScreen(hit->min, hbm) && draw::WorldToScreen(hit->max, hbx))
            {
                ...
                if (draw::WorldToScreen(hit->center, head_scr))
                {
                    ...
                    if (playerlist::AccessData(steamID).state == playerlist::EState::CAT)
    ...
    else if (*healthbar == 3)
    {
        ...
        if (type == ENTITY_PLAYER || type == ENTITY_BUILDING)
        {
            ...
            if (GetCollide(ent))
            {
                ...
                // Draw
                draw::RectangleOutlined(min_x - 7 - *thicc, min_y, 7, max_y - min_y, border, 0.5f);
                draw::Rectangle(min_x - 6 - *thicc, max_y - hbh - 1, 5, hbh, hp);
            }
        }
    }
    ...
    if (ent_data.string_count)
    {
        ...
        if (type == ENTITY_PLAYER || type == ENTITY_BUILDING)
        {
            ...
            if (GetCollide(ent))
            {
                ...
                case 0:
                { // TOP RIGHT
                    draw_point = Vector(max_x + 2 + *thicc, min_y, 0);
                }
                    break;
                case 1:
                { // BOTTOM RIGHT
                    draw_point = Vector(max_x + 2 + *thicc, max_y - data.at(ent->m_IDX).string_count * 16, 0);
                }
                    break;
                case 2:
                { // CENTER
                ...
                case 3:
                { // ABOVE
                    draw_point = Vector((min_x + max_x) / 2.0f, min_y - data.at(ent->m_IDX).string_count * 16 - *thicc, 0);
                }
                    break;
                case 4:
                { // BELOW
                    draw_point = Vector((min_x + max_x) / 2.0f, max_y + *thicc, 0);
                }
                    break;
                case 5:
                { // ABOVE LEFT
                    draw_point = Vector(min_x + 2, min_y - data.at(ent->m_IDX).string_count * 16 - *thicc, 0);
                }
                    break;
                case 6:
                { // ABOVE RIGHT
                    draw_point = Vector(max_x + 2, min_y - data.at(ent->m_IDX).string_count * 16 - *thicc, 0);
                }
                }
            }
        }
...
void _FASTCALL ProcessEntity(CachedEntity *ent)
{
    ...
    else if (ent->m_Type() == ENTITY_PLAYER && ent->m_bAlivePlayer())
    {
        ...
        if (identify_only)
        {
            auto &pl = playerlist::AccessData(ent->player_info.friendsID);
            if (pl.state != playerlist::EState::CAT)
                return;
        }
        ...
        if (ent->m_bEnemy() || teammates || player_tools::shouldAlwaysRenderEsp(ent))
        {
            ...
            if (entity_player_flsim)
                AddEntityString(ent, format("flSimTime ", CE_FLOAT(ent, netvar.m_flSimulationTime)));
...
{	
    ...
    {
        draw::RectangleOutlined(min_x + 1, min_y + 1, max_x - min_x - 2, max_y - min_y - 2, border, *thicc + *thicc2);
        draw::RectangleOutlined(min_x + 1, min_y + 1, max_x - min_x - 2, max_y - min_y - 2, clr, *thicc);
    }
}
