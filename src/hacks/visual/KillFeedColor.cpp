    ...
    if (isHackActive() && enable)
    {
        ...
        if (sort_names)
        {
            ...
            std::sort(displayed_players.begin(), displayed_players.end(), [](player_info_s a, player_info_s b) {
              ...
              static auto getPriority = [](playerlist::EState state) {
                switch (state)
                {
                case playerlist::EState::DEFAULT:
                    return 0;
                case playerlist::EState::CAT:
                case playerlist::EState::FRIEND:
                    return 2;
                case playerlist::EState::RAGE:
                    return 3;
                default:
                    return 1;
                }
...
static InitRoutine init([] {
  ...
  EC::Register(EC::Shutdown, []() { drawtext_detour.Shutdown(); }, "shutdown_killfeed_color");
});
} // namespace hacks::tf2::killfeed
