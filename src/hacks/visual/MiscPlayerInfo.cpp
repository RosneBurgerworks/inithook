...
void Paint()
{
    ...
    for (int i = 0; i <= g_IEngine->GetMaxClients(); i++)
    {
        ...
        if (mafia_city)
        {
            ...
            if (playerlist::AccessData(ent->player_info.friendsID).state == playerlist::EState::CAT)
                color.g = 0.8f;
            ...
            std::string to_display = (playerlist::AccessData(ent->player_info.friendsID).state == playerlist::EState::CAT ? format("Lv.", level, " Cat") : format("Lv.", level, " ", choosen_entry[ent->player_info.friendsID].first));
