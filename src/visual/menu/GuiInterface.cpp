...
static zerokernel::special::PlayerListData createPlayerListData(int userid)
{
    ...
    data.state   = playerlist::pszNames[static_cast<int>(playerlist::AccessData(info.friendsID).state)];
...
static void initPlayerlist()
{
    ...
    if (pl)
    {
        ...
        controller->setChangeStateCallback([](unsigned steam, int userid) {
            ...
            for (unsigned i = 0; i < playerlist::arrGUIStates.size() - 1; i++)
            {
                if (pl.state == playerlist::arrGUIStates.at(i).first)
                {
                    pl.state = playerlist::arrGUIStates.at(i + 1).first;
                    controller->updatePlayerState(userid, playerlist::names[static_cast<size_t>(pl.state)]);
                    return;
                }
            }
            pl.state = playerlist::arrGUIStates.front().first;
            controller->updatePlayerState(userid, playerlist::names[static_cast<size_t>(pl.state)]);
        });
    }
    else
...
bool gui::handleSdlEvent(SDL_Event *event)
{
    ...
    {
        if (event->key.keysym.scancode == SDL_GetScancodeFromKey(SDLK_INSERT))
        {
