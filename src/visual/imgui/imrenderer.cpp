...
font::font(std::string path, int fontsize, bool outline) : size{ fontsize }, new_size{ fontsize }, path{ std::move(std::move(path)) }, outline{ outline }
{
...
void font::rebuild()
{
    ...
    if (!font_new || !font_new->ContainerAtlas)
        return; // what?
    auto *font_config               = (ImFontConfig *) font_new->ConfigData;
...
void Texture::load()
{
    ...
    auto *buffer = new unsigned char[(size_t) size + 1];
...
void renderEnd()
{
    ...
    if (drawdata && drawdata->Valid && buffers[!currentBuffer])
    {
        ...
        {
            ImDrawList *old = dmgr::buffers[!dmgr::curbuf];   // first one should be our
        custom drawlist drawdata->CmdListsCount++;                        // increment to
        accomodate it for (int n = 0; n < drawdata->CmdListsCount; n++) // move all
        pointers up by one and make ours the first entry!
            {
