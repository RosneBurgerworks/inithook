...
static InitRoutine font_size([]() {
    esp_font_size.installChangeCallback([](int after) {
    ...
    center_font_size.installChangeCallback([](int after) {
        if (after > 0 && after < 100)
        {
#if !ENABLE_ENGINE_DRAWING && !ENABLE_IMGUI_DRAWING
...
void Initialize()
{
    ...
    fonts::menu.reset(new fonts::font(DATA_PATH "/fonts/verdana.ttf", 13, true));
    fonts::esp.reset(new fonts::font(DATA_PATH "/fonts/ariblk.ttf", 15, true));
    fonts::center_screen.reset(new fonts::font(DATA_PATH "/fonts/verdana.ttf", 14, true));
#if ENABLE_ENGINE_DRAWING
