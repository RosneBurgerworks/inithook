...
{
    {
        ...
        if (info_text && draw::inited)
        {
            ...
            std::string result = std::string(format_cstr("InitializedHook | %s%s", timeString, server_info.c_str()).get());
            ...
            // Draw!
            draw::Rectangle(*info_x - 5, *info_y - 5, w + 10, h + 10, colors::Transparent(colors::background, *info_alpha));
