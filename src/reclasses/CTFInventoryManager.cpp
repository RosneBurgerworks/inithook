...
bool Craft(std::vector<int> item_ids)
{
    ...
    static DynamicBytePatch patch(patch_addr, { 0x83, 0xc4, 0x6c, 0x5b, 0x5e, 0x5f, 0x5d, 0xc3 });
    patch.Patch();
    ...
    {
        logging::Info("[AutoCraft] %llu", item_UUIDs[i]);
...
bool Rent(int item_id)
{
    ...
    static BytePatch id_patch(id, std::experimental::make_array<uint8_t>(0xa7));
    id_patch.Patch();
