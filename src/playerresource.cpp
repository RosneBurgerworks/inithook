...
static CatCommand debug_level("debug_plr_levels", "Print player levels contained within TFPlayerResource", []() {
    if (!g_IEngine->IsInGame())
        return;

    for (int i = 1; i < 33; ++i)
        logging::InfoConsole("%d: %d", i, g_pPlayerResource->GetLevel(i));
});
