...
CatCommand debug_mantissa("test_mantissa", "For debug purposes", [](const CCommand &rCmd) {
    if (rCmd.ArgC() < 2)
    {
        logging::InfoConsole("You must provide float to test.");
        return;
    }
    ...
    try
    {
        ...
        logging::InfoConsole("Mantissa step for %.3f: %.10f", float_value, mantissa_step);
    }
    catch (const std::invalid_argument &)
    {
        logging::InfoConsole("Invalid float.");
    }
    return;
});
...
static CatCommand nospread_sync("nospread_sync", "Try to sync client and server time", []() {
    if (!bullet)
    {
        logging::InfoConsole("Set nospread.enable to true first.");
        return;
    }
    should_update_time = true;
    no_spread_synced   = NOT_SYNCED;
    logging::InfoConsole("Trying to sync Seed...");
});
...
static CatCommand nospread_resync("nospread_resync", "Try to sync client and server time", []() {
    ...
    {
        logging::InfoConsole("Already syncing.");
        return;
    }
    if (no_spread_synced != SYNCED)
    {
        logging::InfoConsole("Can't resync when not previously synced! Use cat_nospread_sync");
        return;
    }
    no_spread_synced = CORRECTING;
    logging::InfoConsole("Trying to resync Seed...");
});
...
bool DispatchUserMessage(bf_read *buf, int type)
{
    ...
    if (CalculateMantissaStep(vData[0] * 1000.0) < 1.0)
    {
        if (waiting_perf_data)
            logging::InfoConsole("Couldn't sync nospread: server uptime too low.");
    ...
    if (no_spread_synced == NOT_SYNCED)
    {
        ...
        if (debug_nospread)
            logging::InfoConsole("Assumed delta time: %.10f calculated based on %i entries.", float_time_delta, (int) vData.size() - 1);
    ...
    else if (no_spread_synced != SYNCED)
    {
        ...
        if (abs(time_difference) > correction_threshhold || abs(last_correction) > mantissa_step / 1000.0)
        {
            ...
            if (debug_nospread)
                logging::InfoConsole("Applied correction: %.10f", time_difference);
        }
        // We synced successfully
        else
        {
            if (debug_nospread)
                logging::InfoConsole("Nospread successfully synced. Possible precision loss: %.10f Mantissa step: %.2f", time_difference, mantissa_step);
            else
                logging::InfoConsole("Nospread successfully synced. Mantissa step: %.2f", mantissa_step);
            resync_needed = false;
        }
...
void CL_SendMove_hook()
{
    ...
    if (*debug_nospread >= 2)
        logging::InfoConsole("Predicted: %.6f Assumed: %.6f Correction: %.6f Commands: %i", predicted_time, asumed_real_time, write_usercmd_correction, new_packets);
...
static InitRoutine init_bulletnospread([]() {
    ...
    bullet.installChangeCallback([](bool after) {
        if (!after)
        {
            is_syncing       = false;
            no_spread_synced = NOT_SYNCED;
