...
// Array that controls all the payloads for each team. Red team is first, then comes blue
// team.
...
std::optional<Vector> getClosestControlPoint(Vector source, int team)
{
    ...
    int ignore_index = -1;
    if (strstr(g_IEngine->GetLevelName(), "cp_steel"))
    {
        for (int i = 0; i < 4; ++i)
        {
            if (controlpoint_data[i].can_cap.at(team_idx))
            {
                ignore_index = 4;
                break;
            }
