...
static settings::Int vischeck_cache_time("nav.vischeck-cache.time", "300");
static settings::Boolean vischeck_runtime("nav.vischeck-runtime.enabled", "false");
static settings::Int vischeck_time("nav.vischeck-runtime.delay", "3000");
static settings::Int crumb_tollerance("nav.crumb-tollerance", "50");
static settings::Int stuck_time("nav.stuck-time", "200");
// How long it takes to find out we are stuck
...
static settings::Int stuck_blacklist_time("nav.anti-stuck.blacklist-time", "45");
...
static bool CastRay(Vector origin, Vector endpos, unsigned mask, ITraceFilter *filter)
{
    ...
    // This was found to be So inefficient that it is literally unusable for our purposes. it is almost 1000x slower than the
    // above. ray.Init(origin, target, -right * HALF_PLAYER_WIDTH, right * HALF_PLAYER_WIDTH);
...
public:
    ...
    // This is a pure blacklist that does not get cleared and is for free usage internally and externally, e.g. blacklisting
    // where enemies are standing This blacklist only gets cleared on map change, and can be used time independantly. the
    // enum is the Blacklist reason, so you can easily edit it
    ...
    void updateIgnores()
    {
        ...
        if (!update_time.test_and_set(1500))
            return;
            ...
            else
            {
                ...
                for (auto &i : navfile.m_areas)
                {
                    ...
                    if (!IsVectorVisibleNavigation(sticky_origin, area))
                        continue;
                    // Blacklist because it's in range of the sticky, but stickies make no noise, so blacklist it for a
                    // specific timeframe
...
static void followCrumbs()
{
    ...
    if (crumbs[0].vec.DistTo(g_pLocalPlayer->v_Origin) < *crumb_tollerance)
    {
        last_crumb = crumbs[0];
    ...
    else
    {
        ...
        if (!time_spent_on_crumb.check(*stuck_detect_time * 1000))
        {
           ...
           re::C_BaseEntity::EstimateAbsVelocity(RAW_ENT(LOCAL_E), vel);
            // 44.0f -> Revved brass beast, do not use z axis as jumping counts towards that. Yes this will mean long falls
            // will trigger it, but that is not really bad.
    ...
    int weapon_state = CE_GOOD(LOCAL_W) ? CE_INT(LOCAL_W, netvar.iWeaponState) : 0;
    if ((!(g_pLocalPlayer->holding_sniper_rifle && g_pLocalPlayer->bZoomed) && !(LOCAL_W->m_iClassID() == CL_CLASS(CTFMinigun) && weapon_state != MinigunState_t::AC_STATE_IDLE) && crumbs[0].vec.z - g_pLocalPlayer->v_Origin.z > 18 && last_jump.test_and_set(200)) || (last_jump.test_and_set(200) && inactivity.check(*stuck_time / 2)))
    {
...
void checkBlacklist()
{
    ...
    if (current_priority >= melee_enemy || IsPlayerInvulnerable(LOCAL_E))
    {
...
void Draw()
{
    ...
    if (draw_debug_areas && CE_GOOD(LOCAL_E) && LOCAL_E->m_bAlivePlayer())
    {
        ...
        if (draw::WorldToScreen(edge, scrEdge))
            draw::Rectangle(scrEdge.x - 2.0f, scrEdge.y - 2.0f, 4.0f, 4.0f, colors::red_s);
        drawNavArea(area);
    }
...
static CatCommand nav_debug_blacklist("nav_debug_blacklist",
                                      "Blacklist connection between two areas for 30s. First area: cat_nav_set Second area: Your location while running this "
                                      "command.",
                                      []() {
                                          if (!NavEngine::isReady())
                                              return;
...
#endif
    enabled.installChangeCallback([](bool after) {
