...
ItemManager::ItemManager() : mapper()
{
    ...
    RegisterSpecialMapping([](CachedEntity *ent) -> k_EItemType {
        if (ent->m_iClassID() == CL_CLASS(CTFAmmoPack))
            return ITEM_AMMO_MEDIUM;
        return ITEM_NONE;
    });
    RegisterSpecialMapping([](CachedEntity *entity) -> k_EItemType {
        static auto tf2c_weapon_mapper = []() -> ItemModelMapper {
...
void ItemManager::RegisterModelMapping(const std::string &path, k_EItemType type)
{
    mapper.RegisterItem(path, type);
}

void ItemManager::RegisterSpecialMapping(ItemSpecialMapperFn fn)
{
    specials.emplace_back(fn);
}
...
void ItemModelMapper::RegisterItem(const std::string &modelpath, k_EItemType type)
{
...
{
    auto it = models.find(g_IModelInfo->GetModelName(RAW_ENT(entity)->GetModel()));
    if (it == models.end())
