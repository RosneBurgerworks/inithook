...
static settings::Boolean no_reportlimit{ "misc.no-report-limit", "true" };
...
DEFINE_HOOKED_METHOD(PaintTraverse, void, vgui::IPanel *this_, unsigned int panel, bool force, bool allow_force)
{
    ...
    {
        static BytePatch no_report_limit(gSignatures.GetClientSignature,
            "55 89 E5 57 56 53 81 EC ? ? ? ? 8B 5D ? 8B 7D ? 89 D8", 0x75,
            std::experimental::make_array<uint8_t>(0xB8, 0x01, 0x00, 0x00, 0x00));
        no_report_limit.Patch();
        replaced = true;
    }
    call_default = true;
    if (isHackActive() && (hacks::shared::catbot::catbotmode && hacks::shared::catbot::anti_motd && (panel == motd_panel || panel == motd_panel_sd)))
        call_default = false;
